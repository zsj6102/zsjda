...
public class CheckNumView extends EditText {
    ...
    public CheckNumView(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        passwordPaint.setTextSize(30.0f);

        mText = new CharSequence[passwordLength];
    }
...
//            canvas.drawCircle(cx, cy, passwordWidth, passwordPaint);
            canvas.drawText(mText[i].toString(), cx, cy, passwordPaint);
        }
    }
    ...
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        ...
        for (int i = 1; i < textLength + 1; i++) {
            mText[i - 1] = text.subSequence(i - 1, i);
        }
        invalidate();
    }
