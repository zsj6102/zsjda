...
import com.colpencil.propertycloud.Tools.PinyinUtils;
...
public class CityListAdapter extends BaseAdapter {
    ...
    public CityListAdapter(Context mContext, List<TownInfo> mCities) {
        this.mCities = mCities;
        this.inflater = LayoutInflater.from(mContext);
        if (mCities == null) {
            mCities = new ArrayList<>();
        }
        mCities.add(0, new TownInfo("定位", "0"));
    }
        ...
        switch (viewType) {
            ...
            case 1:     //所有
                ...
                if (position >= 1) {
                    ...
                    String currentLetter = PinyinUtils.getFirstLetter(mCities.get(position).getFirst_name());
                    String previousLetter = position >= 1 ? PinyinUtils.getFirstLetter(mCities.get(position - 1).getFirst_name()) : "";
    ...
    public void setLetters() {
        ...
        for (int index = 0; index < size; index++) {
            //当前城市拼音首字母
            String currentLetter = PinyinUtils.getFirstLetter(mCities.get(index).getFirst_name());
            //上个首字母，如果不存在设为""
            String previousLetter = index >= 1 ? PinyinUtils.getFirstLetter(mCities.get(index - 1).getFirst_name()) : "";
            if (!TextUtils.equals(currentLetter, previousLetter)) {
