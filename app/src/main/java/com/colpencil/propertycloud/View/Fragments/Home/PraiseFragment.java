...
public class PraiseFragment extends BaseFragment implements OnClickListener, IComplaintView, PlayerUtil.OnCompleteListener {
        ...
        switch (v.getId()) {
            case R.id.tv_ok: // 确认投诉
                ...
                if (TextUtils.isEmpty(detailDesc) && TextUtils.isEmpty(adiPath)) {
                    ...
                    present.comit(catId, detailDesc, adiPath, 1, lists, recordTime);
                }
                break;
            ...
            case R.id.tv_type:
                ...
                present.getCompType(1, SharedPreferencesUtil.getInstance(getActivity()).getString("comuid"));
                break;
        }
    }
    ...
    public void comit(ResultInfo result) {
        ...
        if (code == 0) {
            MobclickAgent.onEvent(getActivity(), "comitWithSay");
            WarnDialog.showInfoAndIntent(getActivity(), "已收到您的表扬意见，我们将在2小时内处理", ComplaintHistroyActivity.class);
        } else if (code == 1) {
